...
const Accordion = (props) => {
  ...
  const handleClick = (nextIndex) => {
    // check if the item clciked is already set as the open one in our state
    // we need to set an index that will never match an index in our array
   setExpandedIndex((currentExpandedIndex) => {
    if(currentExpandedIndex === nextIndex){ // three equal signs make it care about the type, if they match do:
      return -1
    } else {
      return nextIndex
    }
  ...
  */
  ...
  const renderedItems = items.map((item, index) => {
    const isExpanded = index === expandedIndex
     // condoional expression to switch between two icons if the tab is closed or not
    const icon = (
